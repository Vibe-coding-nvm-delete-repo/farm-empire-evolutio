...
const PlotCard = memo(({ plot, now, onPlotClick }: PlotCardProps) => {
  ...
  const roundedPrevNow = Math.floor(prevProps.now / 1000)
  const roundedNextNow = Math.floor(nextProps.now / 1000)
  
  return (
    prevProps.plot.id === nextProps.plot.id &&
    prevProps.plot.type === nextProps.plot.type &&
    prevProps.plot.cropId === nextProps.plot.cropId &&
    prevProps.plot.animalId === nextProps.plot.animalId &&
    prevProps.plot.buildingId === nextProps.plot.buildingId &&
    prevProps.plot.completesAt === nextProps.plot.completesAt &&
    roundedPrevNow === roundedNextNow
  )
})
...
export function FarmGrid({ plots, onPlotClick, onCollectAll, onBulkPlantRow }: FarmGridProps) {
  ...
  const rows = useMemo(() => {
    return [0, 1, 2, 3, 4].map(rowIndex => {
      ...
      return { rowIndex, rowPlots, emptyInRow }
    })
  }, [plots])
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="grid grid-cols-[auto_repeat(5,1fr)] gap-3 px-4">
        <TooltipProvider>
          {rows.map(({ rowIndex, rowPlots, emptyInRow }) => (
            <div key={`row-${rowIndex}`} className="contents">
              ...
              {rowPlots.map(plot => (
                <PlotCard key={plot.id} plot={plot} now={now} onPlotClick={onPlotClick} />
              ))}
            </div>
          ))}
        </TooltipProvider>
